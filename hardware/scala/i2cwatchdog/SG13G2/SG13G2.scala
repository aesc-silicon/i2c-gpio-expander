...
case class SG13G2Top(p: I2cWatchdog.Parameter, resetDelay: Int) extends Component {
  val io = new Bundle {
  ...
  io.i2c.interrupt <> IOPadOut4mA(system.watchdog.io.i2c.interrupts(0))
  io.i2c.scl <> IOPadInOut4mA(system.watchdog.io.i2c.scl)
  io.i2c.sda <> IOPadInOut4mA(system.watchdog.io.i2c.sda)
  for (index <- 0 until io.gpio.length) {
    io.timeouts(index) <> IOPadOut4mA(system.watchdog.io.timeouts(index))
  }
}
object SG13G2Generate extends ElementsApp {
  elementsConfig.genASICSpinalConfig.generateVerilog {
    val top = SG13G2Top(I2cWatchdog.Parameter.default.copy(addressWidth=3), 128)
    ...
    io.addPad("west", 2, "sg13g2_IOPadIOVss")
    io.addPad("west", 3, "sg13g2_IOPadIOVdd")
    io.generate(top.io)
